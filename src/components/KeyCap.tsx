...
export function KeyCap({ index }: { index: number }) {
  ...
  return (
    <button
      ...
      `}
    >
      <span className="font-medium overflow-hidden whitespace-nowrap max-w-full">{tapLabel}</span>
      {holdLabel && (
        <span className="text-[9px] text-muted-foreground overflow-hidden whitespace-nowrap max-w-full">{holdLabel}</span>
      )}
      {hasMorph && (
